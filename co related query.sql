...
from employees
where (salary,deptid) in (select avg(salary),deptid from employees group by deptid);


-- corelated  a corelated query is a subquery that depends on the outer query for its values and is execute repeatedly for each row processed by the outer query
...
SELECT s.StudentID,s.StudentName,s.Department AS StudentDepartment,c.CourseName
FROM Students s
...
SELECT 
    ...
    LAST_DAY(p.payment_date),
    p.amount
FROM payment p
JOIN customer c ON p.customer_id = c.customer_id
WHERE YEAR(p.payment_date) > 2004
  AND p.amount > (SELECT AVG(p2.amount)FROM payment p2
